...
void cpuEmbedding(const int *input_ids, float *output, float *embed_table, const int max_context_token_num, const int hidden_units, const int vocab_size) {
    for (int i = 0; i < max_context_token_num; ++i) {
        for (int j = 0; j < hidden_units; ++j) {
            output[j + i * hidden_units] = embed_table[j + input_ids[i] * hidden_units];
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    const int table_size = vocab_size * hidden_units;
    const int output_size = max_context_token_num * hidden_units;
    ...
    if (argv[1]) {
        ...
        for (int i = 0; i < table_size; ++i) {
            h_table[i] = (float)(i / hidden_units);
        }
        ...
        TensorWrapper<float>* output = new TensorWrapper<float>(Device::GPU, type_float, {max_context_token_num,     hidden_units}, d_output);
        ...
        for (int i = 0; i < max_context_token_num; i++){
            std::cout << (float)h_output[i * hidden_units] << std::endl;
        }
        ...
        DataType type_int = getTensorType<int>();
        TensorWrapper<int>* input_ids = new TensorWrapper<int>(Device::GPU, type_int, {max_context_token_num},    d_input);
        TensorWrapper<half>* output = new TensorWrapper<half>(Device::GPU, type_half, {max_context_token_num,     hidden_units}, d_output);
