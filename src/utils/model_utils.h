...
namespace llm {
    ...
    BaseModel *CreateModelWithName(const std::string& model_name) {
      LLM_CHECK_WITH_INFO(model_name ==
        "llama", "dont support other models except llama yet!");
      int head_num = 32;
      int kv_head_num = 32;
      int head_size = 128;
      ...
      BaseModel *model = new Llama<T>(head_num, kv_head_num, head_size,
                              inter_size, num_layers, vocab_size,
                              attn_static_params, max_seq_len, stream,
                              cublas_wrapper, allocator, &deviceProp);
      return model;
    }
  ...
  template<typename T>
  std::unique_ptr<BaseModel> CreateRealLLMModel(
    std::string model_dir, std::string tokenizer_file) {
    BaseModel *model = CreateModelWithName<T>("llama");
