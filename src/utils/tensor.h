...
// data会被放在TensorWrapper 中, 想对TensorWrapper进行模板化
struct Tensor 
{
    Device              location;
    DataType            dtype;
    ...
    // static_cast: dynamic是最安全的，但是使用static, 因为我们可以确保这个tensor就是TensorWrapper
    template<typename T>
    TensorWrapper<T>* as() {
    ...
    TensorWrapper(Device location, DataType dtype, std::vector<int> shape, T* data):
        Tensor(location, dtype, shape), data(data) {
            ...
            LLM_CHECK_WITH_INFO(in_dtype == dtype, 
                "when build TensorWrapper, the passed in data type should be same as dtype in params");
        }
