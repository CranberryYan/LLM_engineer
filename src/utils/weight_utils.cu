...
// from FT code
// loads data from binary file. If it succeeds,
//	returns a non-empty (shape size) vector. If loading fails or
// 	the product of the elements in shape is 0,
...
std::vector<T> loadWeightFromBinHelper(std::vector<size_t> shape,
	...
	if (size == 0) {
		std::cout << "shape is zero, skip loading weight from file: "
			<< filename << std::endl;
		return std::vector<T>();
	}
	...
	if (!in.is_open()) {
		std::cout << "file" << filename << "cannot be opened, loading model fails!"
			<< std::endl;
		return std::vector<T>();
	}
	...
	std::cout << "Read " << std::to_string(loaded_data_size) << " bytes from "
		<< filename << std::endl;
...
public:
	static void internalFunc(T_OUT* ptr, std::vector<size_t> shape,
		std::string filename) {
		std::vector<T_FILE> host_array =
			loadWeightFromBinHelper<T_FILE>(shape, filename);
		if (host_array.empty()) {
			return;
		}
