...
void Llama<T>::allocateCPUBuffer(int batch_size) {
  // 13:
  //  context_ids = {1, 18637, 29892, 526, 366, 19861,
  //    29973, 1815 , 366, 5193, 304, 592, 29973};
  h_input_ids_buf_ =
...
void Llama<T>::allocateGPUBuffer(int batch_size) {
  ...
  // for context decoder
  context_decoder_input = new TensorWrapper<T>(GPU, getTensorType<T>(),
    {/*token num*/ 13, hidden_units});
  context_decoder_output = new TensorWrapper<T>(GPU, getTensorType<T>(),
    {/*token num*/ 13, hidden_units});

  // split from context_decoder_output
  context_decoder_lmhead_input = new TensorWrapper<T>(GPU, getTensorType<T>(),
    {/*token num*/ 1, hidden_units});

  // for self decoder
  decoder_input = new TensorWrapper<T>(GPU, getTensorType<T>(), 
    {/*batch size*/ 1, hidden_units});
  decoder_output = new TensorWrapper<T>(GPU, getTensorType<T>(),
    {/*batch size*/ 1, hidden_units});
  input_ids = new TensorWrapper<int>(GPU, getTensorType<int>(),
    {/*token num*/ 13});

  // for context decoder
  input_length = new TensorWrapper<int>(GPU,getTensorType<int>(),
    {batch_size});
  history_length = new TensorWrapper<int>(GPU, getTensorType<int>(),
    {batch_size});
  context_length = new TensorWrapper<int>(GPU, getTensorType<int>(),
    {batch_size});
  sequence_lengths = new TensorWrapper<int>(GPU, getTensorType<int>(),
    {batch_size});

  // kv cache buffer
  all_k_cache = new TensorWrapper<T>(GPU, getTensorType<T>(),
    {num_layers, batch_size, kv_head_num, max_seq_len, head_size});
  all_v_cache = new TensorWrapper<T>(GPU, getTensorType<T>(),
    {num_layers, batch_size, kv_head_num, max_seq_len, head_size});
  token_ids = new TensorWrapper<int>(GPU, getTensorType<int>(),
    {batch_size});
  is_finished = new TensorWrapper<bool>(GPU, getTensorType<bool>(),
    {batch_size});
  output_rmsnorm_weight = new TensorWrapper<T>(GPU, getTensorType<T>(),
    {hidden_units}, llama_weights->out_rmsnorm_weight.gamma);
  probs = new TensorWrapper<T>(GPU, getTensorType<T>(),
    {batch_size, vocab_size});
  unused_residual = new TensorWrapper<T>(GPU, getTensorType<T>(),
    {batch_size, hidden_units});

  // allocate buffer of above
  unused_residual->data = allocator->Malloc(unused_residual->data,
    sizeof(T) * 13 * hidden_units, false);;
  context_decoder_input->data = allocator->Malloc(context_decoder_input->data,
    sizeof(T) * 13 * hidden_units, false);
  context_decoder_output->data = allocator->Malloc(context_decoder_output->data,
    sizeof(T) * 13 * hidden_units, false);
  context_decoder_lmhead_input->data = allocator->Malloc(
    context_decoder_lmhead_input->data, sizeof(T) * 1 * hidden_units, false);
  decoder_input->data = allocator->Malloc(decoder_input->data,
    sizeof(T) * batch_size * hidden_units, false);
  decoder_output->data = allocator->Malloc(decoder_output->data,
    sizeof(T) * batch_size * hidden_units, false);

  input_ids->data = allocator->Malloc(input_ids->data, sizeof(int) * 13, false);
  input_length->data = allocator->Malloc(input_length->data,
    sizeof(int) * batch_size, false);
  history_length->data = allocator->Malloc(history_length->data,
    sizeof(int) * batch_size, false);
  context_length->data = allocator->Malloc(context_length->data,
    sizeof(int) * batch_size, false);
  sequence_lengths->data = allocator->Malloc(sequence_lengths->data,
    sizeof(int) * batch_size, false);

  all_k_cache->data = allocator->Malloc(all_k_cache->data,
    sizeof(T) * num_layers * batch_size * max_seq_len * kv_head_num * head_size, false);
  all_v_cache->data = allocator->Malloc(all_v_cache->data,
    sizeof(T) * num_layers * batch_size * max_seq_len * kv_head_num * head_size, false);
  token_ids->data = allocator->Malloc(token_ids->data,
    sizeof(int) * batch_size, false); // 4, 进位到32x为32
  is_finished->data = allocator->Malloc(is_finished->data,
    sizeof(bool) * batch_size, false); // 4, 进位到32x为32
  probs->data = allocator->Malloc(probs->data,
    sizeof(T) * batch_size * vocab_size, false);

  // topK buffer
  topk_id = new TensorWrapper<int>(GPU, getTensorType<int>(),
                  {batch_size, beamwidth, BlockPerBeam, K});
  topk_val = new TensorWrapper<T>(GPU, getTensorType<T>(),
                    {batch_size, beamwidth, BlockPerBeam, K});
  final_topk_id = new TensorWrapper<int>(GPU, getTensorType<int>(),
                        {batch_size * beamwidth, K});
  final_topk_val = new TensorWrapper<T>(GPU, getTensorType<T>(),
    {batch_size * beamwidth, K});
  topk_id->data = allocator->Malloc(topk_id->data,
    sizeof(int) * batch_size * beamwidth * BlockPerBeam * K, false);
  topk_val->data = allocator->Malloc(topk_val->data,
    sizeof(T) * batch_size * beamwidth * BlockPerBeam * K, false);
  final_topk_id->data = allocator->Malloc(final_topk_id->data,
    sizeof(int) * batch_size * beamwidth * K, false);
  final_topk_val->data = allocator->Malloc(final_topk_val->data,
    sizeof(T) * batch_size * beamwidth * K, false);
}
...
// 返回所有轮次总共的input, 总共input的history部分, 总共input的当前轮次input部分
template <typename T>
std::vector<std::string> Llama<T>::MakeInput(const std::string &history,
  int round, const std::string &input) {
  std::vector<std::string> ret = {(round == 0 ? "" : history) + input,
    history, input};
  return ret;
}
...
void Llama<T>::InitializeForContextDecoder(IntDict &int_params_first_token) {
  ...
  CHECK(cudaMemcpy(input_ids->data, h_input_ids_buf_,
          sizeof(int) * h_input_length_buf_[0], cudaMemcpyHostToDevice));
  CHECK(cudaMemcpy(input_length->data, h_input_length_buf_,
          sizeof(int) * batch_size, cudaMemcpyHostToDevice));
  CHECK(cudaMemcpy(history_length->data, h_history_length_buf_,
          sizeof(int) * batch_size, cudaMemcpyHostToDevice));
  CHECK(cudaMemcpy(context_length->data, h_context_length_buf_,
          sizeof(int) * batch_size, cudaMemcpyHostToDevice));
  CHECK(cudaMemcpy(is_finished->data, h_finished_buf_,
          sizeof(bool) * batch_size, cudaMemcpyHostToDevice));
}
...
template <typename T>
void Llama<T>::inputEmbedding(TensorWrapper<int> *input_ids,
  TensorWrapper<T> *decoder_input) {
    launchInputEmbedding<T>(input_ids, decoder_input,
      &(llama_weights->pre_decoder_embedding_weight));
    DeviceSyncAndCheckCudaError();
}
...
int Llama<T>::LMHeadAndTopKSample(TensorMap &decoder_outputs) {
  ...
  if (index == 0) {
    ...
    auto ptr = decoder_output_tensorwrapper->data +
      (input_length - 1) * hidden_units;
    context_decoder_lmhead_input->data = ptr;

    launchLinearGemm(
      // [1, hidden units] for ctx decoder
      /*Tensor**/ context_decoder_lmhead_input,
      // lm_head.weight.bin, [vocab_size, hidden_units]
      /*BaseWeight&*/ llama_weights->post_decoder_embedding_weight,
      //[1, vocab size] for context decoder
      /*Tensor**/ probs, cublas_wrapper, false, true);
      DeviceSyncAndCheckCudaError();
  } else {
    // for self decoder
    launchLinearGemm(
      //[bs, hidden_units] for self decoder
      /*Tensor**/ decoder_output->as<T>(),
      // lm_head.weight.bin, [vocab_size,hidden_units]
      /*BaseWeight&*/ llama_weights->post_decoder_embedding_weight,
      //[bs, vocab size] for self decoder
      /*Tensor**/ probs, cublas_wrapper, false, true);
    DeviceSyncAndCheckCudaError();
  }
...
template <typename T>
int Llama<T>::firstTokenGen(LLaMaAttentionDynParams &dparams,
  ...
  LLM_CHECK_WITH_INFO(context_decoder_input->data != nullptr,
    "GPU context decoder input data is not initialized");
  LLM_CHECK_WITH_INFO(history_length->data != nullptr,
    "GPU history_length data is not initialized");
  LLM_CHECK_WITH_INFO(input_length->data != nullptr,
    "GPU input_length data is not initialized");
  LLM_CHECK_WITH_INFO(context_length->data != nullptr,
    "GPU context_length data is not initialized");
  LLM_CHECK_WITH_INFO(output_rmsnorm_weight->data != nullptr,
    "GPU output_rmsnorm_weight data is not initialized");
  ...
  TensorMap decoder_inputs{
    ...
    {"output_norm_weight", output_rmsnorm_weight},
    {"layer_id", layer}};
  ...
  context_decoder->forward(decoder_inputs, llama_weights->llama_layer_weight,
    decoder_outputs, dparams);
...
int Llama<T>::continueTokenGen(LLaMaAttentionDynParams &dparams) {
  ...
  // context decoder里面RoPE输出的k和v写到kv cache
  // self decoder之后每一个step都会输出kv到kv cache,
  //  需要保证kv cache是llama class的成员, 这样就可以保证同步更新
  TensorMap decoder_outputs{
...
// 单轮对话, batch_size = 1
template <typename T>
std::string Llama<T>::Response(const std::vector<std::string> &input,
  CallBack PrintRes) {
  // prompt = "Hey, are you conscious? Can you talk to me?"
  std::vector<int> res = {1, 18637, 29892, 526, 366, 19861,
    29973, 1815, 366, 5193, 304, 592, 29973};
  
  // 总共input的history部分
  std::string history_str = input[1]; // history
  ...
  // 返回所有轮次总共的input
  std::string total_str = input[0]; // (round == 0 ? "" : history) + input
  ...
  // 一个batch中的q的最大长度，因为此时不支持batch，所以就等于cur_input_length
  attn_dyn_params.max_q_len = attn_dyn_params.num_tokens;

  // max_context_length, 指当前batch的动态最大上下文长度
  attn_dyn_params.max_k_len = context_length;

  // step最初为context_length, 在self_decoder的阶段才会用到
  ...
  while (index < 20) /*self define output_token_limit*/ {
    // KV_cache here input is empty, only buffer, output is not empty
    if (index == 0) {
    ...
    *(step->data) = *(step->data) + 1;
    ...
    if (index == 0) {
      TensorWrapper<int> tmp =
        TensorWrapper<int>(CPU,getTensorType<int>(), {1}, &ret);
      LLM_CHECK(tmp.shape != input_ids->shape);
      LLM_CHECK(tmp.dtype == input_ids->dtype);
      LLM_CHECK(tmp.location != input_ids->location);
      allocator->Free(input_ids->data);
      input_ids->data =
        allocator->Malloc(input_ids->data, sizeof(int) * 1, false);
      input_ids->shape = {1};
      CHECK(cudaMemcpy(input_ids->data, tmp.data,
        sizeof(int) * 1, cudaMemcpyHostToDevice));
    } else {
      CHECK(cudaMemcpy(input_ids->data, &ret,
        sizeof(int) * 1, cudaMemcpyHostToDevice));
    }
    index++;
  }
