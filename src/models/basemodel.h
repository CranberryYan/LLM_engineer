...
public:
  ...
  // 用于定义每轮对话的输入、历史记录和回复API, 每个具体模型子类需要实现
  // 根据历史信息和当前输出生成当前轮次的prompt
  virtual std::vector<std::string> MakeInput(const std::string &history,
    int round, const std::string &input) = 0;
  // 根据当前轮次回复更新history
  virtual std::string MakeHistory(const std::string &history, int round,
    const std::string &input, const std::string &output) = 0;
  // 回复内容的返回接口
  virtual std::string Response(const std::vector<std::string> &intput,
    CallBack PrintRes) = 0;
};