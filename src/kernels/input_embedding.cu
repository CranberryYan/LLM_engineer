...
__global__ void embeddingFunctor(const int *input_ids, T *output,
    const T *embed_table, const int max_context_token_num, const int hidden_units) {
    // gid (每个thread负责output的一个元素)
    int index = blockDim.x * blockIdx.x + threadIdx.x;
    for (int i = index; i < max_context_token_num * hidden_units; 
        i += blockDim.x * gridDim.x) {
        // input: [num_tokens]
        // output: [num_tokens, hidden_units]
        // id: 行号
        // i % hidden_units: 列号
        int id = input_ids[i / hidden_units]; // id: 输入token的id
        output[index] = embed_table[id * hidden_units + i % hidden_units];
    }
}
...
void launchInputEmbedding(
    ...
    TensorWrapper<T> *output,       // FP32 [max_context_token_num, hidden_units] = [token_num, 4096] -> [多少个词, 每个词用多少维向量表示]
    EmbeddingWeight<T>* embed_tabel // FP32 [vocab_size, hidden_units] -> [词汇表的大小, 每个词用多少维向量表示]
) {
    ...
    const int hidden_units           = output->shape[1];
    ...
    embeddingFunctor<T><<<gird, block>>>(
        input_ids->data, output->data, embed_tabel->data, max_context_token_num, hidden_units);
}
