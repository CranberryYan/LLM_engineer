...
// ctx qkv linear: [num_tokens, qhidden_units] * [qhidden_unis, hidden_untis]
// ctx attn output linear: [num_tokens, head_num, head_size] * [qhidden_units, qhidden_units] // qhidden_units = head_num * head_size
// self qkv linear: [bs, qhidden_units] * [qhidden_units, hidden_units] = [bs, head_num, head_size, hidden_units]
// self attn output linear: [bs, qhidden_units] * [qhidden_units, qhidden_units]
// lmhead linear: [bs, qhidden_units] * [vocab_size, qhidden_units](转置)
        ...
        bool trans_a, bool trans_b)
{
    ...
    cublas_wrapper->Gemm(transA, transB, 
                        trans_b ? Ak : Am, Cn, Bk, 
                        weight.data, lda, input->data, ldb, output->data, ldc, 1.0f, 0.0f);
}
        ...
        cublasWrapper* cublas_wrapper, bool trans_a, bool trans_b)
{
    ...
    cublas_wrapper->strideBatchGemm(transA, transB, Cn, Cm, Bk, 
