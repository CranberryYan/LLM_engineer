/*
kv cache出现动机：
    pagedAttention是对kv cache所占空间的分页管理，是典型的以内存空间换计算开销的手段。
    Decoder推理中，对于每个输入的prompt，在计算第一个token输出的时候，每个token的attention都要从头计算，
但是后续的token生成中，需要concat前面每一个token的K和V，由于模型参数矩阵是不变的，
此时只有刚生成的那个token的K和V需要从头计算，所以可以把之前token的K和V缓存起来避免重复计算，这个就叫kv cache。

显存占用大小：
    每个decoder layer中，每个token的k、v矩阵都是embedding_size = num_heads * head_size，
...
// src shape: [bs, kv_head_num, max_q_len, head_size] 为什么这里不是max_k_len?
//  主要因为 q k v = from_tensor * qkv_weight
//  from_tensor: 输入的token经过embedding和RMSNorm后的tensor
// kv cache shape: [num_layers, bs, kv_head_num, max_seq_len(8192), head_size] ->
...
// after RoPE: k/v shape: [bs, kv_head_num, max_q_len, head_size]
template <typename T>
