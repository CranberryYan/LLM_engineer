...
void LlamaLayerWeight<T>::loadWeights(std::string weight_path,
  ...
  loadWeightFromBin<T, float>::internalFunc(attn_norm_weight.gamma,
    {hidden_units}, weight_path + ".input_layernorm.weight.bin");
  loadWeightFromBin<T, float>::internalFunc(ffn_norm_weight.gamma,
    {hidden_units}, weight_path + ".post_attention_layernorm.weight.bin");

  // 2. attention_weights
  loadWeightFromBin<T, float>::internalFunc(self_attn_weight.qkv.data,
    {(head_num + 2 * kv_head_num) * head_size, hidden_units},
    weight_path + ".self_attn.qkv.weight.bin");
  loadWeightFromBin<T, float>::internalFunc(self_attn_weight.output.data,
    {hidden_units, hidden_units}, weight_path + ".self_attn.o_proj.weight.bin");

  // 3. ffn_weights
  loadWeightFromBin<T, float>::internalFunc(ffn_weight.gateAndup.data,
    {2 * inter_size, hidden_units},
    weight_path + ".mlp.gate_up_proj.weight.bin");
  loadWeightFromBin<T, float>::internalFunc(ffn_weight.down.data,
    {hidden_units, inter_size}, weight_path + ".mlp.down_proj.weight.bin");

  // 4. bias
  if (attn_bias) { //TODO: Llama2没有bias
    loadWeightFromBin<T, float>::internalFunc(self_attn_weight.qkv.bias,
      {(head_num + 2 * kv_head_num) * head_size},
      weight_path + ".attention.wqkv.bias.bin");
    loadWeightFromBin<T, float>::internalFunc(self_attn_weight.output.bias,
      {head_num *  head_size}, weight_path + ".attention.wo.bias.bin");
  } else {
